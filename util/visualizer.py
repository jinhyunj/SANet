...
def save_images(save_dir, visuals, image_path, aspect_ratio=1.0):
    ...
    I_g_fake = util.tensor2im(visuals['I_g_fake'])

    im_name = '%s.png' % name
    save_path = os.path.join(save_dir, im_name)
    util.save_image(I_g_fake, save_path, aspect_ratio=aspect_ratio)
...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, epoch_iter, save_result):
        ...
        if save_result or not self.saved:  # save images to an HTML file if they haven't been saved.
            self.saved = True
            Gs = torch.cat((visuals['I_g'], visuals['I_g_fake']), 2)
            AGs = torch.cat((visuals['I_a'], Gs), 3)
            AGs_im = util.tensor2im(AGs)
            img_path = os.path.join(self.img_dir, 'epoch%.3d_iter%d.png' % (epoch, epoch_iter))
            util.save_image(AGs_im, img_path)
