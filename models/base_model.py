...
class BaseModel(ABC):
    ...
    def save_networks(self, epoch, latest=False):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                if latest:
                    save_filename = 'latest_net_%s.pth' % name
                else:
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                for key in list(state.keys()):  # need to copy keys here because we mutate in loop
                    self.__patch_instance_norm_state_dict(state, net, key.split('.'))
                net.load_state_dict(state['state_dict'])
                #net.load_state_dict(torch.load('./segnet.pth'))
            epoch_count = state['epoch']
